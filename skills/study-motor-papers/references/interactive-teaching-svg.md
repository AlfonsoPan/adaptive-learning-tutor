...
配套文件：`../assets/interactive-teaching-svg-template.svg` 是可直接渲染的最小完整骨架。必须保留其中的 1200×688 根节点、`<style>`、`<defs>` active-glow 滤镜、底部交互面板样式和末尾 `<script>` IIFE 的基线行为；按内容替换 topics 字典和图形元素。只有加入滑杆等真实交互控件时才扩展脚本，扩展不得删减 pointer 四事件、捕获/释放、键盘、复位与静态首帧一致性逻辑。
...
4. **写文件**：以 `../assets/interactive-teaching-svg-template.svg` 为骨架，保留根节点、style/defs/面板样式和脚本基线行为；填入内容元素与 topics 字典，并让内容充分使用画布。
